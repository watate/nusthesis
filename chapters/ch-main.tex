...
\begin{figure}[h]
  \centering
  \includegraphics[width=.9\linewidth]{\Pic{png}{paper_Simple World_Success Rate_lineraw}}
  \vspace{\BeforeCaptionVSpace}
  \caption{Graph of Episode Result vs Time. Episode result is 1 if the agent successfully reaches the target, and 0 if it fails.}
  \label{main:visualization_difficulty_1}
\end{figure}
\newpage

It is difficult to see how well the model is performing over time from ~\autoref{main:visualization_difficulty_1} because the agent constantly alternates between success and failure. Therefore, we have chosen to use an exponentially weighted moving average (EWMA) to better visualize how the agent's performance is improving over time (as well as other variables which we will look at in this work). EWMA is calculated as follows:
\begin{equation}
...
While it is possible to pass $\alpha$ directly, it's often easier to think about the span of an EW moment. In this work, unless otherwise specified, we use a span of 1000. This is also known as an "N-day EW moving average". That is:
\begin{equation}
\alpha = \frac{2}{s + 1} \text{ for span s $\geq$ 1}
\end{equation}


Using EWMA, our graph of episode result vs time is now transformed into a graph of success rate vs time:

\begin{figure}[h]
  \centering
  \includegraphics[width=.9\linewidth]{\Pic{png}{paper_Simple World_Success Rate_linesmoothed}}
  \vspace{\BeforeCaptionVSpace}
  \caption{Graph of Success Rate vs Time.}
...
In this work, we measure performance as the return of a trajectory (which is a measure of cumulative reward along the trajectory). The way this is computed is according to ~\autoref{main:eq:performance}:
...
~\autoref{main:performance_1} shows that all 3 models display performance improvements at approximately equal rates. 
\begin{figure}[h]
  \centering
  \includegraphics[width=.9\linewidth]{\Pic{png}{paper_Simple World_Success Rate_linesmoothed}}
  \vspace{\BeforeCaptionVSpace}
  \caption{Average Performance vs Time}
